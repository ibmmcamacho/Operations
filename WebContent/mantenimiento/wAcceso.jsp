						...
						function(pdata) {
							...
							if (obj.ListaTFC[0].squad == "") {
								...
								for (i = 0; i < obj.ListaTFC.length; i++) {
									...
									var ticket = "<a target='_blank' href='http://ism.ecampus.com.pe/maximo/ui/maximo.jsp?event=loadapp&value=" + obj.ListaTFC[i].clase + "&uniqueid="+ obj.ListaTFC[i].unique_id +"'>" + obj.ListaTFC[i].ticket + "</a>";
	...
	$.post(src,{
		...
		strOwnerId : strOwnerId
					...
					function(pdata) {
						...
						// DataTable
						$('#dtPlanned tfoot th').each( function () {
						        var title = $(this).text();
						        $(this).html( '<input class="dtPlannedtext" type="text" placeholder="'+title+'" />' );
						} );

						var table2 = $('#dtPlanned').DataTable({
							data : datatabla2,
							order: [[3, 'asc']], 
							processing : true,
							"columnDefs": [
					    ...
					    table2.columns().every( function () {
					        ...
					        $('.dtPlannedtext', this.footer() ).on( 'keyup change', function () {
					            if ( that.search() !== this.value ) {
					                that.search( this.value ).draw();
...
// 			alert(idPerfil + ' || ' + Descripcion);
	...
	});"WebContent/template/common/wtcHeader.jsp"
	

	function listarComboCliente(){
	   $('#cboDispatchCliente option').remove();			
	   $('#cboDispatchCliente').append($('<option>', {value: '0',text : "Todos"}));
	   
	   var src = "ServletReporteDispatch";
	   $.post(src, {strOperacion : 'listParentCliente'},
			function(pdata) {
				var obj = JSON.parse(pdata);
